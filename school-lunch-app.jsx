...
import { Calendar, Heart, Share2, TrendingUp, Star, Search, Sparkles } from 'lucide-react';

// ==========================================
// 🔑 API 키 설정
// ==========================================
// 아래에 NEIS 오픈 API에서 발급받은 인증키를 입력하세요.
// 
// 📝 API 키 발급 방법:
// 1. https://open.neis.go.kr 접속
// 2. 회원가입 및 로그인
// 3. '인증키 신청' 메뉴에서 신청
// 4. 발급받은 키를 아래 YOUR_API_KEY_HERE 부분에 붙여넣기
// 
// 예시: const NEIS_API_KEY = 'abc123def456ghi789jkl012mno345pqr678stu901';
// ==========================================

const NEIS_API_KEY = 'YOUR_API_KEY_HERE';
...
const SchoolLunchApp = () => {
  ...
  // 로컬스토리지에서 데이터 불러오기
  useEffect(() => {
    const savedFavorites = localStorage.getItem('schoolLunchFavorites');
  ...
  const highlightAllergens = (text) => {
    const allergens = ['대두', '밀', '우유', '계란', '땅콩', '새우', '게', '오징어', '고등어', '돼지고기', '복숭아', '토마토'];
  ...
  const fetchMealData = async (date) => {
    ...
    try {
      ...
      if (data.mealServiceDietInfo && data.mealServiceDietInfo[1]) {
        ...
        // 중식만 필터링
        const lunchData = meals.find(meal => meal.MMEAL_SC_NM === '중식') || meals[0];
        ...
        setMealData(null);
        alert('해당 날짜의 급식 정보가 없습니다.');
      }
    } catch (error) {
      console.error('Error fetching meal data:', error);
      alert('데이터를 불러오는데 실패했습니다. 학교를 다시 선택해주세요.');
    }
    setLoading(false);
  };
  ...
  const shareMeal = () => {
    ...
    const shareText = `${mealData.SCHUL_NM} ${selectedDate.toLocaleDateString('ko-KR')} 급식\n${mealData.DDISH_NM.replace(/<br\/>/g, '\n')}`;
  ...
  const saveDefaultSchool = () => {
    ...
    alert(`${selectedSchool.name}이(가) 기본 학교로 설정되었습니다!`);
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-8 pt-8 relative">
          <h1 className="text-5xl font-bold gradient-text mb-3 flex items-center justify-center gap-3">
            ...
            맛있는 급식
            <span className="emoji-float" style={{animationDelay: '1s'}}>✨</span>
          </h1>
          <p className="text-gray-600 text-lg">오늘의 점심 메뉴를 확인해보세요!</p>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl">
              <div className="text-center mb-6">
                ...
                <h2 className="text-3xl font-bold gradient-text mb-2">맛있는 급식에 오신 것을 환영합니다!</h2>
                <p className="text-gray-600">학교 급식 정보를 쉽고 예쁘게 확인하세요</p>
              </div>
              
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium mb-2 text-gray-700">자주 보는 학교 (선택)</label>
                  <input
                    type="text"
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold gradient-text">⚙️ 설정</h2>
                <button
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium mb-2 text-gray-700">현재 선택된 학교</label>
                  <div className="px-4 py-3 bg-gray-100 rounded-xl">
                    {selectedSchool ? selectedSchool.name : '선택된 학교 없음'}
                  </div>
                </div>
                ...
                <div className="bg-blue-50 p-4 rounded-xl">
                  <p className="text-xs text-gray-700 leading-relaxed">
                    💡 기본 학교를 설정하면 앱을 열 때마다<br/>
                    해당 학교의 급식이 자동으로 로드됩니다!
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-3xl shadow-xl p-6 mb-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div className="relative">
                  <label className="block text-sm font-medium mb-2 text-gray-700">🏫 학교 검색</label>
                  <input
                    type="text"
                    value={schoolName}
                    onChange={(e) => {
                  ...
                  {selectedSchool && (
                    <div className="mt-2 px-3 py-2 bg-green-50 border-2 border-green-200 rounded-xl text-sm">
                      <strong className="text-green-700">✓ 선택됨:</strong> {selectedSchool.name}
                    </div>
                  )}
                </div>
                ...
                {/* 날짜 선택 */}
                <div>
                  <label className="block text-sm font-medium mb-2 text-gray-700">📅 날짜 선택</label>
                  <input
                    type="date"
              ...
              <div className="flex gap-3 flex-wrap">
                ...
                {mealData && (
                  <>
                    ...
                    >
                      <Heart size={20} fill={favorites.some(f => f.schoolCode === selectedSchool?.schoolCode) ? 'white' : 'none'} />
                      {favorites.some(f => f.schoolCode === selectedSchool?.schoolCode) ? '즐겨찾기됨' : '즐겨찾기'}
                    </button>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                ...
                <div className="lg:col-span-2 bg-white rounded-3xl shadow-xl p-8 meal-card">
                  <div className="flex justify-between items-start mb-6">
                    ...
                    <div className="text-4xl emoji-float">🍽️</div>
                  </div>
                  ...
                  <div className="mt-6 p-4 bg-gradient-to-r from-yellow-50 to-orange-50 rounded-xl">
                    <h4 className="font-bold mb-3 text-gray-800">오늘 급식 어땠나요?</h4>
                    <div className="flex gap-3 justify-center">
                      {[
                        { emoji: '😋', label: '맛있어요', value: 'good' },
                        { emoji: '😐', label: '보통이에요', value: 'normal' },
                        { emoji: '😫', label: '별로예요', value: 'bad' }
                      ].map(rating => (
                        <button
                          key={rating.value}
                ...
                <div className="space-y-6">
                  ...
                  {mealData.CAL_INFO && (
                    <div className="bg-white rounded-3xl shadow-xl p-6">
                      <h3 className="text-xl font-bold mb-4 gradient-text">🔥 칼로리 정보</h3>
                      <div className="text-center">
                        ...
                        <div className="text-sm text-gray-500 mt-2">
                          권장 섭취량 대비 {Math.round((parseFloat(mealData.CAL_INFO) / 800) * 100)}%
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {mealData.NTR_INFO && (
                    <div className="bg-white rounded-3xl shadow-xl p-6">
                      <h3 className="text-xl font-bold mb-4 gradient-text">📊 영양소</h3>
                      <div className="space-y-3">
                        {(() => {
                          ...
                          const colors = {
                            '탄수화물': 'from-blue-400 to-blue-500',
                            '단백질': 'from-red-400 to-red-500',
                            '지방': 'from-yellow-400 to-yellow-500',
                          };
                  ...
                  {mealData.ORPLC_INFO && (
                    <div className="bg-white rounded-3xl shadow-xl p-6">
                      <h3 className="text-xl font-bold mb-4 gradient-text">🌍 원산지</h3>
                  ...
                  {weeklyData.map((item, idx) => (
                    ...
                    >
                      ...
                      {item.data ? (
                        ...
                        <div className="text-xs text-gray-400 text-center">급식없음</div>
                      )}
                    </div>
                  ))}
                </div>
                      ...
                      <div key={idx} className="flex items-center gap-4">
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between mb-1">
                            ...
                            <span className="font-bold text-purple-600">{count}번</span>
                          </div>
                          <div className="bg-gray-200 rounded-full h-3 overflow-hidden">
                            <div 
